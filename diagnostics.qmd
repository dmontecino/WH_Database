...
In the data model, **each Diagnostic has a unit of measurement that can either be qualitative or quantitative. The result of the diagnostic is reported as a value of those units. The interpretation, based on the result on those units, can either be positive, negative, or undetermined**. **It is ideal to include supporting information to each Diagnostic, such as digital images or files to corroborate the entered result in the database**. 

The Interpretation of a Diagnostic **should adhere to the case definition criteria established in the corresponding Surveillance Activity metadata for each declared method**. 

**Diagnostics can produce "Diagnostic Products" that are treated as a Specimen** in the data model for two main reasons. The first one is the possibility to track the location of the product, the quantities available, and any exportation process. The second reason is because these Diagnostics Products can be used in further Diagnostics. For example, the cDNA from a RT-PCR assay that is used as material for a new chain reaction.
