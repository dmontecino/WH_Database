# Starting point
target_uri <- c("https://lod.nal.usda.gov/nalt/18060", 
                "https://lod.nal.usda.gov/nalt/26359")
...
cardio_uris<-unique(current_uris)
...
circulatory_nodes <- full_nalt_node[node_uri %in% cardio_uris]
...
uri_nodes <- xml_attr(circulatory_nodes, "about")
...
preferred_labels <- extract_preferred_labels(circulatory_nodes)
...
create_circulatory_table <- function(labels, uris, definitions, alt_labels) {
...
# Create final table
circulatory_table <- create_circulatory_table(
  preferred_labels, 
  uri_nodes, 
  definitions, 
  alt_labels_list
...
circulatory_table<-
  circulatory_table |> 
  filter(!(Topography %in% c("Arterioles", "Blood platelets", "Erythrocytes",
                           "Blood capillaries", "B-lymphocytes", "Lymphocytes",
                           "Sarcomeres", "Granulocytes", "Blood cells", "Heterophils",
                           "T-lymphocytes", "Natural killer cells", "Cardiomyocytes", 
                           "Neutrophils", "Myofibrils", "Plasma cells", "Cd8-positive t-lymphocytes",
                           "Leukocytes", "Basophils", "Suppressor cells", "Microvessels",
                           "Cytotoxic t-lymphocytes", "Mononuclear leukocytes", 'Monocytes', 
                           "Reticulocytes", "Cd4-positive t-lymphocytes", "Immunocytes", 
                           "Myofibrillar proteins", "Eosinophils", "Endothelium", "Hemocytes")))
