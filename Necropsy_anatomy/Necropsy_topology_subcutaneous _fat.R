...
sc_fat_uris<-"https://lod.nal.usda.gov/nalt/18338"
...
sc_fat_nodes <- full_nalt_node[node_uri %in% sc_fat_uris]
...
uri_nodes <- xml_attr(sc_fat_nodes, "about")
...
preferred_labels <- extract_preferred_labels(sc_fat_nodes)
...
create_sc_fat_table <- function(labels, uris, definitions, alt_labels) {
...
# Create final table
sc_fat_table <- create_sc_fat_table(
  preferred_labels, 
  uri_nodes, 
  definitions, 
  alt_labels_list
