...
sinuses_uris<-c("https://lod.nal.usda.gov/nalt/18329",
                "https://lod.nal.usda.gov/nalt/56825")
...
# Extract eye nodes
sinuses_nodes <- full_nalt_node[node_uri %in% sinuses_uris]
...
uri_nodes <- xml_attr(sinuses_nodes, "about")
...
preferred_labels <- extract_preferred_labels(sinuses_nodes)
...
create_sinuses_table <- function(labels, uris, definitions, alt_labels) {
...
# Create final table
sinuses_table <- create_sinuses_table(
  preferred_labels, 
  uri_nodes, 
  definitions, 
  alt_labels_list
