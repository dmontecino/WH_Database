...
target_uri <- c("https://lod.nal.usda.gov/nalt/8971", 
                "https://lod.nal.usda.gov/nalt/4351")
...
endo_nodes <- full_nalt_node[node_uri %in% endo_uris]
...
uri_nodes <- xml_attr(endo_nodes, "about")
...
preferred_labels <- extract_preferred_labels(endo_nodes)
...
endo_table<-
  endo_table |> 
  filter(Topography %in% c("Posterior pituitary", 
                             "Thyroid gland",
                             "Parathyroid glands",
                             "Pituitary gland",
                             "Endocrine system",
