# Starting point
target_uri <- c("https://lod.nal.usda.gov/nalt/20541")
...
lympha_uris<-unique(current_uris)
...
lympha_nodes <- full_nalt_node[node_uri %in% lympha_uris]
...
uri_nodes <- xml_attr(lympha_nodes, "about")
...
preferred_labels <- extract_preferred_labels(lympha_nodes)
...
create_table <- function(labels, uris, definitions, alt_labels) {
...
lympha_table<-
  lympha_table |> 
  filter(!(Topography %in% c("Macrophages", "Neutrophils", "Histiocytes",
                             "Foam cells", "Reticuloendothelial system", "Monocytes",
