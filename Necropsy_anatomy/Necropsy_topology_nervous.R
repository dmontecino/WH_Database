...
target_uri <- c("https://lod.nal.usda.gov/nalt/19220", 
                "https://lod.nal.usda.gov/nalt/18339",
                "https://lod.nal.usda.gov/nalt/1617")
...
nervo_uris<-unique(current_uris)
...
nervo_nodes <- full_nalt_node[node_uri %in% nervo_uris]
...
uri_nodes <- xml_attr(nervo_nodes, "about")
...
preferred_labels <- extract_preferred_labels(nervo_nodes)
...
nervo_table<-
  nervo_table |> 
  filter(!(Topography %in% c("Visual cortex", 
                           "Mushroom bodies",
                           "Antennal lobe"))) 
