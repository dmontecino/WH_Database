# Starting point
target_uri <- c("https://lod.nal.usda.gov/nalt/5853")
...
resp_nodes <- full_nalt_node[node_uri %in% resp_uris]
...
uri_nodes <- xml_attr(resp_nodes, "about")
...
preferred_labels <- extract_preferred_labels(resp_nodes)
...
create_resp_table <- function(labels, uris, definitions, alt_labels) {
...
resp_table<-
  resp_table |> 
  filter(!(Topography %in% c("Nose", "Pneumocytes", "Pulmonary alveoli",
                             "Nasal mucosa", "Nasal cavity", "Paranasal sinuses")))
