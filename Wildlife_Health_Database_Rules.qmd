...
The animals of belonging to an Observation must be categorized by sex, age and 
health status. 
...
If the animals can be identified, they are observed multiple times, and they are 
followed longitudinally, they should be added as individual animals and tracked 
...
information (e.g., individual stranded pilot whales), because the
Animal Sources provide Specimens to perform Diagnostics or to be stored (carcass,
body part, tissue, organs, samples, etc), or because a Diagnostic is conducted in
individual animals that do not provide Specimens (e.g., evident hair loss in a 
free-ranging red fox).

Immutable data of Animal Sources are the sex and the species.

#### Animal Source Record

Each Animal Source has at least one Source Record and if they are individually 
identified and tracked over time, they can have as many Source Records
as needed.  

Mutable data of Animal Sources are the captivity category, the health status and 
disease symptoms, and the age.

The last Source Record for an animal is not necessarily the last time t when it
was found dead because it is possible to collect Specimens from the carcass (dead 
Animal Source) over a period of time while the carcass is rooting. 

#### Necropsies

Necropsies are associated with a specific Animal Source Record, when the carcass
was collected, but the Necropsy date can be any date since the Animal Source Record
creation.
...
Record, but the date of the Necropsy (primary or secondary). It is possible to track
if a Specimen was collected during the primary or secondary Necropsy based on the 
information collected in Specimens (see below).
