...
in a live animal up to a trace of tissue to conduct a highly technological assay.
Invertebrate Source Records can have multiple species; however, it is expected that
invertebrates in their Specimens belong to the same species (if the Specimen 
contains more than one invertebrate).
...
and the current amount can differ if certain quantity of the specimen has already
been used for previous Diagnostics.
...
different. The amount of the Specimen should be 'full animal'. 

Specimens whose current amount is zero must not be deleted and the data of their
last location should remain in the database.
...
that is the same among Specimens sharing the same container. The Specimen identifier
of each Specimen within the Container should be different.
