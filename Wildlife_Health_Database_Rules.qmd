...
for a full description of each type). The Source types available depend on the 
Surveillance Objective methods and goals. Following the modular properties of the 
database, Source types become available on a Surveillance Objective basis per user
request.

Each Source is recorded at least once (Source Records) but depending on their type
they could be recorded multiple times over time (e.g.,capture-mark-recapture studies
with pathogen testing at each capture instance). Most Sources can provide Specimens
such as full carcasses, body parts, fluids, tumors, blood, water, sediment, 
invertebrates, etc.
...
Animal Source is an animal in an Incident providing individual-level health
data. Each Animal Source has at least one Source Record  
