...
![](Pictures_and_diagrams/group_sources.png){width=100%}
...
![](Pictures_and_diagrams/animal_sources.png){width=100%}
**Figure 10**: Detailed relationships of Animal Source in the data model. 

<!-- Key attributes of an **Animal Source** include: -->
...
<!-- The data model allows the inclusion of vaccinations administered to an **Animal Source Record**. Multiple vaccination records can be added to an **Animal Source Record** as needed, ensuring that vaccination history accumulates. -->
...
muscle_table<- muscle_table[muscle_table$Topography != "musculoskeletal system",]
# 
# muscle_table |> 
#   mutate(Topography = stringr::str_to_sentence(Topography)) |> 
#   mutate(Alternative_name = stringr::str_to_sentence(Alternative_name)) |> 
...
circulatory_table<-circulatory_table[circulatory_table$Topography!="Cardiovascular system",]
...
resp_table<-resp_table[resp_table$Topography!="Respiratory system",]
...
sinuses_table<-sinuses_table[sinuses_table$Topography!="Respiratory system",]
...
digestive_table<- digestive_table[digestive_table$Topography !=  "Gastrointestinal system",]
...
repro_table<-repro_table[!(repro_table$Topography %in% c("Female reproductive system", "Male reproductive system", "Reproductive system")),]
...
lympha_table<-lympha_table[lympha_table$Topography !="Lymphatic system",]
...
endo_table<- endo_table[endo_table$Topography != "Endocrine system",]
...
nervo_table <- nervo_table[nervo_table$Topography != "Central nervous system", ]
...
![](Pictures_and_diagrams/arthropod_sources.png){width=100%}
...
![](Pictures_and_diagrams/environmental_sources.png){width=100%}
